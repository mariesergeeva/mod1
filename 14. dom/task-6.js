...
document.addEventListener("keydown", (event) => {
  ...
  isDarkThemeOn = darkTheme(keydown, isDarkThemeOn)
})
function darkTheme(key, theme) {
  if (!theme && key === 'Tab') theme = true
  else if (theme && key === 'Tab') theme = false
  if (theme) {
    document.body.style.background = '#24292E'
    let taskItem = document.querySelectorAll('.task-item')
    taskItem.forEach(item => item.style.color = '#ffffff')
    let allButtons = document.querySelectorAll('button')
    allButtons.forEach(item => item.style.border = '1px solid #ffffff')
  } else {
    document.body.style.background = 'initial'
    let taskItem = document.querySelectorAll('.task-item')
    taskItem.forEach(item => item.style.color = 'initial')
    let allButtons = document.querySelectorAll('button')
